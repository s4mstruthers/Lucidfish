...
Lucidfish reviews your chess games like a coach. The Stockfish engine finds what
happened; an AI language model (running on your own computer, or a cloud model if
you prefer) explains it in plain chess concepts — development, king safety, forks,
pins, pawn structure — pitched at your level and linked to the mistakes you keep
making across games.
...
- **A modern web app.** Live progress while the game is analysed, an interactive board
  with arrows and an evaluation bar, a clickable evaluation graph, variation previews,
  coach chat, and light and dark themes. It also works on phones and tablets.
...
- **Efficient.** Engine results and opening lookups are cached, so re-analysing a game is
  nearly instant. Engine and AI work overlap, and cloud models answer several moves in parallel.
...
2. **Analyze games:** choose chess.com or Lichess to list your recent games and click one, or
   tick several and click *Analyze selected* to analyse them in the background. You can also
   paste or drop a PGN file.
3. **Review the game:** moves appear while the analysis runs. Click any move, or use
   <kbd>←</kbd>/<kbd>→</kbd> (<kbd>Home</kbd>/<kbd>End</kbd> to jump, <kbd>F</kbd> to flip).
   Each move shows its verdict, tactical tags, the coach's note, better alternatives, and the
   line that punishes a mistake. Click a line to play it out on the board, and press
   <kbd>Esc</kbd> to return. Click the evaluation graph to jump to a moment.
4. **Ask the coach** about the position you're looking at, or about your overall review on the
   dashboard.
5. **Export** an annotated PGN or a Markdown report from the game header.
6. **Board editor:** set up any position (or paste a FEN) and ask for an assessment from either
   side's point of view.

The **dashboard** shows your record, average accuracy, blunders per game, your openings and
how you score in them, and the coach's written review of your progress.
...
| `--detail key\|standard\|full`, `--explain-all` | How much the coach writes (`--explain-all` = `full`) |
...
| `LUCIDFISH_DETAIL` | `standard` | `key`, `standard` or `full` |
...
- **Notes are fact-checked.** If a note mentions a move that isn't legal and appears in none of
  the given lines, or a piece on a square where there is none, the AI is asked once to correct
  itself.
...
| `lucidfish/pipeline.py` | Orchestrates engine and coach per move; game review; position mode |
...
- Training drills generated from your own recurring mistakes
